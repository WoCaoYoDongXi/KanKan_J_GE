...
# 定義區域（你可以換成你研究的火山位置）
roi = ee.Geometry.Point([-175.2049470, -21.1988048]).buffer(20000)

# 載入 MODIS AOD 產品
modis_aod = ee.ImageCollection('MODIS/006/MOD04_L2').select('Optical_Depth_Land_And_Ocean')

# 定義兩個時間範圍
before_start = '2021-09-16'
before_end = '2021-09-20'
after_start = '2022-04-21'
after_end = '2022-04-25'

# 計算期間的平均 AOD
modis_aod_before = modis_aod.filterDate(before_start, before_end).filterBounds(roi).mean().clip(roi)
modis_aod_after = modis_aod.filterDate(after_start, after_end).filterBounds(roi).mean().clip(roi)
...
Map.split_map(
    left_layer=geemap.ee_tile_layer(modis_aod_before, aod_vis, 'AOD Before'),
    right_layer=geemap.ee_tile_layer(modis_aod_after, aod_vis, 'AOD After')
)
...
# 顯示到 Streamlit
st.title("火山噴發前後的 AOD 比較")
st.markdown("**左圖：** 噴發前（2021/09/16-20）\n\n**右圖：** 噴發後（2022/04/21-25）")
